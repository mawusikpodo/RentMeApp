...
const Detail = () => {
  ...
  const { data: appartment } = useQuery(
    "fetchAppartmentById",
    () => apiClient.fetchAppartmentById(appartmentId || ""),
    {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold">{appartment.name}</h1>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {appartment.imageUrls.map((image) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-2">
        {appartment.facilities.map((facility) => (
